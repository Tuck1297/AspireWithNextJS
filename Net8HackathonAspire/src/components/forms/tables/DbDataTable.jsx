...
const Table = ({
  ...
  const handleUpdate = (index) => {
    ...
    setTimeout(async () => {
      await externalDbService
        .updateRow(baseUrl, currentDBInteracting.id, oldRowData, newRowData)
        .then((result) => {
          ...
          const updateToDisplayIndex = findObjInArrayIndex(
            oldRowData,
            toDisplayData
          );
          updatedToDisplayData[updateToDisplayIndex] = {
            ...updatedToDisplayData[updateToDisplayIndex],
            ...editRowData,
          };
  ...
  const handleDelete = (deleteData) => {
    ...
    setTimeout(async () => {
      await externalDbService
        .deleteRow(baseUrl, currentDBInteracting.id, deleteData)
        .then((result) => {
  ...
  const handleAddRow = () => {
    ...
    setTimeout(async () => {
      await externalDbService
        .createRow(baseUrl, currentDBInteracting.id, newRowData)
        .then((result) => {
  ...
  return (
    <>
      ...
      ></Modal>
      <AIModal
        tableRows={currentTableInteracting.rowsInTable}
        aiModalOpen={aiModalOpen}
        setAiModalOpen={setAiModalOpen}
        currentDbInteracting={currentDBInteracting}
      />
      <div className="d-flex justify-content-center flex-column">
        <h2 className="text-center mt-3 mb-3 text-decoration-underline">
          {currentTableInteracting.tableName} Table In{" "}
          {currentDBInteracting.dbName} Database
        </h2>
